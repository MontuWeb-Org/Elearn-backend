...
- **Password-reset OTPs are cache, not a table.** Three calls: `POST /auth/password/forgot` emails a hashed 6-digit code (TTL 10 minutes, 5 failed checks evict the key). `POST /auth/password/otp/verify` consumes the OTP and writes a one-time `reset_token` (TTL restarts 10 minutes). `POST /auth/password/reset` takes `email` + `reset_token` + `password`. There is no `PASSWORD_RESET_OTPS` row. Always `202` on forgot so a missing account is not distinguishable from a sent code. A new forgot replaces the live cache entry.
